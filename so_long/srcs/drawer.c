...
/*   Updated: 2022/07/23 16:19:42 by seonggyk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player(t_game_props *props, int x, int y)
{
	mlx_put_image_to_window(props->mlx,
		props->win, props->imgs[GRASS], IMG_SIZE * x, IMG_SIZE * y);
	mlx_put_image_to_window(props->mlx,
		props->win, props->imgs[PLAYER], IMG_SIZE * x, IMG_SIZE * y);
}
...
void	draw_chest(t_game_props *props, int x, int y)
{
	mlx_put_image_to_window(props->mlx,
		props->win, props->imgs[GRASS], IMG_SIZE * x, IMG_SIZE * y);
	mlx_put_image_to_window(props->mlx,
		props->win, props->imgs[CHEST], IMG_SIZE * x, IMG_SIZE * y);
}
